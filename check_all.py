from pionex_api_template import send_pionex_request

res = send_pionex_request("GET", "/api/v1/account/balances")
balances = res['data']['balances']

total_usdt_value = 0.0
breakdown = []

for b in balances:
    coin = b['coin']
    free = float(b['free'])
    if free <= 0:
        continue
        
    if coin == 'USDT':
        total_usdt_value += free
        breakdown.append((coin, free, free))
    else:
        symbol = f"{coin}_USDT"
        try:
            ticker_res = send_pionex_request("GET", "/api/v1/market/tickers", params={"symbol": symbol})
            if ticker_res and 'data' in ticker_res and len(ticker_res['data']['tickers']) > 0:
                price = float(ticker_res['data']['tickers'][0]['close'])
                val = free * price
                total_usdt_value += val
                if val > 0.01:
                    breakdown.append((coin, free, val))
        except Exception as e:
            pass

print(f"\nTOTAL ACCOUNT VALUE: ${total_usdt_value:.2f}\n")
for coin, amount, val in sorted(breakdown, key=lambda x: x[2], reverse=True):
    print(f"{coin}: {amount} (${val:.2f})")
